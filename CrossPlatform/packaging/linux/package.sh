...
if $release; then
    ...
    : "${LOCALFLOW_GPG_FINGERPRINT:?LOCALFLOW_GPG_FINGERPRINT is required for a signed release}"
    : "${LOCALFLOW_GPG_PASSPHRASE:?LOCALFLOW_GPG_PASSPHRASE is required for a signed release}"
    expected_fingerprint="$(tr -d '[:space:]' <<<"$LOCALFLOW_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]')"
...
rm -f -- "${dist_dir}/SHA256SUMS" "${dist_dir}/SHA256SUMS.asc" \
    "${dist_dir}/UNSIGNED-NOT-FOR-DISTRIBUTION.txt"
(
    cd "$dist_dir"
    sha256sum \
